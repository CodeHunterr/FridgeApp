...
- `appsettings.json`: mevcut SQL Server bağlantı ayarı. İçindeki gerçek secret temizlenmeli.
...
- `appsettings.json` içinde gerçek placeholder olmayan SQL Server bağlantı bilgisi bulunuyor.
- Bu secret temizlenmeden repo GitHub'a taşınmamalı.
- `appsettings.Development.json` şu an sadece logging içeriyor.
- User secrets, `.env` veya production secret store entegrasyonu henüz kurulmuş değil.

## 23. SQL Server'dan PostgreSQL/Neon'a geçerken dikkat edilmesi gerekenler
- `Microsoft.EntityFrameworkCore.SqlServer` sağlayıcısı `Npgsql.EntityFrameworkCore.PostgreSQL` ile değişecek.
- `Program.cs` içindeki `UseSqlServer` çağrısı `UseNpgsql` olacak.
- SQL Server provider'ı ile üretilen migration'lar Neon için doğrudan doğru kabul edilmemeli.
...
## 24. Migration durumu
Repo içinde şu migration'lar var:
...
Notlar:
- `AppDbContextModelSnapshot.cs` mevcut.
- Repo'da migration dosyalarının bulunması, her ortamda veritabanına uygulanmış oldukları anlamına gelmez.
- Yeni bilgisayarda veya Neon ortamında `database update` ayrıca çalıştırılmalıdır.

## 25. Bilinen riskler
- `appsettings.json` içinde temizlenmesi gereken gerçek bir DB secret var.
...
- SQL Server -> PostgreSQL geçiş branch'i henüz açılmamış.
...
3. Dockerfile ekle.
4. SQL Server provider'ından `Npgsql` provider'ına geç.
5. Postgres uyumlu migration stratejisini netleştir.
6. Neon veritabanını ayağa kaldır ve connection string'i env variable olarak bağla.
7. Basit bir `/health` endpoint ekle.
8. Koyeb üzerinde GitHub bağlantılı deploy kur.
9. Deploy sonrası temel endpoint smoke testlerini çalıştır.
10. Flutter mobil proje ile yeni backend URL uyumluluğunu kontrol et.
